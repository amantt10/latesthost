...
const RSSectionThree = ({ formData, updateFormData, step }: Props) => {
  const handleSelect = (value: string) => {
    updateFormData("is_ms_resident", value);
  };

  const getDropdownTitle = () => {
    const found = YES_NO_OPTIONS.find((o) => o.value === formData.is_ms_resident);
  ...
  return (
    <div className="max-w-xl w-full mx-auto rounded-none md:rounded-2xl p-4 md:p-8 shadow-input bg-white">
      <h2 className="font-bold text-2xl text-center text-[#5d1725] dark:text-neutral-200 mb-5">
        Section 3 - Mississippi Residency
      </h2>
      <form className="my-8">
